...
3. Run `docker pull seankross/bologna`
5. To start the RStudio server `docker run --name=rstudiocon -e USER=<username> -e PASSWORD=<password> -dp 8787:8787 seankross/bologna`.  You may also do `-e ROOT=TRUE`, but you still should have USER/PASSWORD.  Then log into `YOURIP:8787` using the username and password. For making multiple users, see https://github.com/rocker-org/rocker/wiki/Using-the-RStudio-image#multiple-users.  
